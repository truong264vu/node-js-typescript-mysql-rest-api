...
class QuestionRepository  implements IQuestionRepository {
  save(questions: Question): Promise<Question> {
    return new Promise((resolve, reject) => {
      ...
      "INSERT INTO questions (questions, audio, date ,created_at, url_video) VALUES(?,?,?,?,?)",
      [JSON.stringify(questions.questions), JSON.stringify(questions.audio), format(questions.date, 'yyyy-MM-dd'), new Date(), questions.url_video],
      (err, res) => {
        if (err) reject(err);
        else
  ...
  update(question: Question): Promise<number> {
    return new Promise((resolve, reject) => {
      connection.query<OkPacket>(
        "UPDATE questions SET questions = ?, audio = ?, date = ?, url_video = ? WHERE id = ?",
        [JSON.stringify(question.questions), JSON.stringify(question.audio), format(question.date, 'yyyy-MM-dd'), question.url_video, question.id],
        (err, res) => {
          console.log(err);
          if (err) reject(err);
